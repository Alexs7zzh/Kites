...
function createDotPath(svg, key, x, y, index) {
...
function initMagneticShell(root) {
    ...
    let anchorYPositions = [];
    let decorativeLineYPixels = [];
    let rightDecorativeLines = [];
    let leftElements = [];
    let dotRefsArray = Array.from({ length: LINE_COUNT }, () => []);
    let dotYPositions = [];
    ...
    function resetDots() {
        for (const path of svgElement.querySelectorAll('path[data-dot-key]')) {
            path.remove();
        }
        ...
        dotYPositions = Array.from({ length: LINE_COUNT }, (_, lineIndex) => Array.from({ length: dotCount }, () => anchorYPositions[lineIndex] ?? 0));
        draggedYPositions = [...anchorYPositions];
    }
    function renderDecorativeDots() {
        for (const path of svgElement.querySelectorAll('path[data-decorative="true"]')) {
            path.remove();
        }
        ...
        leftElements.forEach((element, elementIndex) => {
            ...
            element.dotPositions.forEach((xPosition, dotIndex) => {
                const path = createDotPath(svgElement, `left-${elementIndex}-${dotIndex}`, xPosition, yPosition, dotIndex);
                path.dataset.decorative = 'true';
            });
        });
    }
        ...
        LEFT_ELEMENT_Y_POSITIONS_PERCENT.forEach((percent, index) => {
            ...
            if (buttonIndex !== -1 && buttonIndex < navButtons.length) {
                ...
                elements.push({
                    ...
                    y,
                    underlineY: y + 20,
                    dotPositions: calculatePiecewiseDotPositions(underlineStartX, underlineEndX, Math.max(2, Math.floor(underlineWidth / BUTTON_DOT_SPACING))),
                });
                return;
            }
            elements.push({
                ...
                type: 'deco',
                y,
                dotPositions: calculatePiecewiseDotPositions(leftElementXStart, leftDecoLineEndX, Math.max(2, Math.floor((leftDecoLineEndX - leftElementXStart) / BUTTON_DOT_SPACING))),
            });
        });
    ...
    function updateNavLayout() {
        ...
        const firstButton = leftElements.find((element) => element.type === 'button');
        if (!firstButton) {
            return;
        }
    ...
    function calculateLayout() {
        ...
        rightDecorativeLines = decorativeLineYPixels.map((yPosition) => ({
            y: yPosition,
            dotPositions: calculatePiecewiseDotPositions(rightLinesXStart, windowWidth, Math.max(2, Math.floor((windowWidth - rightLinesXStart) / BUTTON_DOT_SPACING))),
        }));
        const navShiftProgress = Math.min(1, Math.max(0, (windowWidth - NAV_LOGO_SHIFT_END) / (DESKTOP_NAV_LOGO_SHIFT_START - NAV_LOGO_SHIFT_END)));
    ...
    function onFrame() {
        ...
        const dots = dotCount;
        const startStateX = windowWidth * (isMobile ? 0.7 : RIGHT_DECORATIVE_LINE_X_START_PERCENT) - WAVY_LINE_RIGHT_MARGIN;
        let allMetaballsPath = '';
        for (let lineIndex = 0; lineIndex < LINE_COUNT; lineIndex += 1) {
            if (dots === 0 || !decorativeLineYPixels[decorativeLineYPixels.length - LINE_COUNT + lineIndex])
                continue;
            ...
            if (lineIndex === 0)
                currentLeftY = anchorYPositions[lineIndex];
            ...
            const waveComponent = x.map((_, dotIndex) => {
                ...
                const sine = Math.sin(currentFrequency * ratio + frame * currentWaveSpeed + lineIndex * 0.15) * ratio;
